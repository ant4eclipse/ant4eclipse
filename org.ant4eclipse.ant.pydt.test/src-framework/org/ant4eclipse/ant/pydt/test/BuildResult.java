...
public class BuildResult {
  ...
  private static final String      VAR_WORKSPACE = String.format( "${%s}", AntProperties.PROP_WORKSPACEDIR );
  ...
  private List<String>             _workspacepathes;
  ...
  public BuildResult( File workspacedir, String dirseparator ) {
    ...
    _workspacepathes = new ArrayList<String>();
    _workspacepathes.add( abspath.replace( '\\', '/' ) );
    _workspacepathes.add( abspath.replace( '/', '\\' ) );
    _workspacepathes.add( Utilities.replace( abspath.replace( '\\', '/' ), "/", dirseparator ) );
    try {
      abspath = workspacedir.getCanonicalPath();
      _workspacepathes.add( abspath.replace( '\\', '/' ) );
      _workspacepathes.add( abspath.replace( '/', '\\' ) );
      _workspacepathes.add( Utilities.replace( abspath.replace( '\\', '/' ), "/", dirseparator ) );
  ...
  public String[] getTargetOutput( String target ) {
    List<String> content = _collectedbytarget.get( target );
    if( content == null ) {
      return new String[0];
    }
  ...
  private void addLine( List<String> list, String line ) {
    for( String wspath : _workspacepathes ) {
      line = Utilities.replace( line, wspath, VAR_WORKSPACE );
    }
    list.add( line );
  }
