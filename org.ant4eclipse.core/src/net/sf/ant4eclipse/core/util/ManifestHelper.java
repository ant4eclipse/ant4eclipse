...
public class ManifestHelper {
  ...
  public static ManifestHeaderElement[] getManifestHeaderElements(final String manifestValue) {
    ...
    for (int i = 0; i < elements.length; i++) {
      ...
      final String[] elementParts = splitHeader(elements[i], ";");

      for (int j = 0; j < elementParts.length; j++) {
        String[] splitted = splitHeader(elementParts[j], ":=");
        // Directive found...
        if (splitted.length > 1) {
          manifestHeaderElement.addDirective(splitted[0], removeQuotes(splitted[1]));
        } else {
          splitted = splitHeader(elementParts[j], "=");
          if (splitted.length > 1) {
            manifestHeaderElement.addAttribute(splitted[0], removeQuotes(splitted[1]));
          } else {
            manifestHeaderElement.addValue(elementParts[j]);
          }
        }
      }
    }

    return (ManifestHeaderElement[]) result.toArray(new ManifestHeaderElement[0]);
  }
