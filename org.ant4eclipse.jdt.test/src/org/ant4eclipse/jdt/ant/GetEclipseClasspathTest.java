...
public class GetEclipseClasspathTest extends AbstractJDTBuildFileTest {
  ...
  public void setUp() throws Exception {
    super.setUp();
    File simpleProjectDir = JdtEclipseProjectBuilder.getPreConfiguredJdtBuilder("simpleproject").createIn(
        getWorkspaceDir());
    _simpleProjectBinDir = new File(simpleProjectDir, "bin");

    // projectB depends on simpleProject but doesn't re-export it
    File projectbDir = JdtEclipseProjectBuilder.getPreConfiguredJdtBuilder("projectb").withSrcClasspathEntry(
        "/simpleproject", false).createIn(getWorkspaceDir());
  ...
  public void test_WithMultipleReferences() throws Exception {
    ...
    // projectC references b. Since b doesn't reexport simple project, simpleproject will be invisible
  ...
  public void test_WithMultipleReferencesRuntime() throws Exception {
    ...
    // projectC references b. Since b doesn't reexport simple project, anyway: simpleproject will be visible
