 ...
 * 
 */
public class GetJdtBuildOrderTask extends AbstractTeamProjectSetBasedTask {

  /** Comment for <code>_buildorderProperty</code> */
  private String                 _buildorderProperty;

  /**
   * Takes a list of project names - separated by comma - that should be computed
   */
  ...
  /**
   * Returns the location of the Target Platform that should be used to resolve the build order of <b>plugins</b>. May
   * return null.
   * 
  ...
  /**
   * Sets the location of the Target Platform that should be used to resolve the build order of <b>plugins</b>. May be
   * null. If the location is null, the workspace is used as the only target platform. If not null, it must point to an
   * existing directory.
   */
  ...
  /**
   * Ensures, that the targetPlatformLocation parameter has been set correctly, i.e. either be null or point to a
   * directory.
   * 
   * @throws BuildException
   *           if the parameter is set incorrectly (i.e. not null and not a directory)
   * 
   */
  ...
  protected void doExecute() {
    ...
    try {
      ...
      if (isProjectNamesSet()) {
        ...
        final EclipseProject[] projects = getWorkspace().getAllProjects();
      ...
      orderedProjects = BuildOrderResolver.resolveBuildOrder(getWorkspace(), getTargetPlatformLocation(), projectNames,
          getNonJavaProjectHandling().asBuildOrderResolverConstant());

      getProjectSetBase().setStringProperty(this._buildorderProperty, convertToString(orderedProjects, ','));
   ...
   * @param separator
   *          The separation character. Sequences of separators will be ignored.
   * 
   * @return A list of splitted strings. The elements will be trimmed. Not null.
   */
   ...
   */
  private String convertToString(final EclipseProject[] projects, final char separator) {
