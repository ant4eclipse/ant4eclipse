...
public class ExecuteJdtProjectTask extends AbstractProjectBasedTask {
	...
	protected void doExecute() {
		...
		// TODO: ASSERT
		final JavaProjectRole javaProjectRole = JavaProjectRole.Helper
				.getJavaProjectRole(eclipseProject);

		// TODO: CONTAINER ARGS
		final ResolvedClasspath resolvedClasspath = ClasspathResolver
				.resolveProjectClasspath(getEclipseProject(), false, false,
						new Properties());

		final File[] cp = resolvedClasspath.getClasspathFiles();
		final String resolvedpath = TaskHelper.convertToString(cp,
				File.pathSeparator, File.separator, getProject());
		...
		for (final MacroDefinitionWrapper macroDefinitionWrapper : this._macroDefs) {
			...
			if (macroDefinitionWrapper.isSourceDirectoryScope()) {
				executeSourceDirectoryScopedMacroDef(macroDef, javaProjectRole,
						resolvedpath);
			} else if (macroDefinitionWrapper.isTargetDirectoryScope()) {
				executeOutputDirectoryScopedMacroDef(macroDef, javaProjectRole,
						resolvedpath);
			}
		}
	}
	...
	private void executeSourceDirectoryScopedMacroDef(final MacroDef macroDef,
			final JavaProjectRole javaProjectRole, final String cp) {
		...
		for (final String sourceFolder : sourceFolders) {
			...
			scopedProperties.put("output.directory", javaProjectRole
					.getOutputFolderForSourceFolder(sourceFolder));
			scopedProperties.put("default.output.directory", javaProjectRole
					.getDefaultOutputFolder());
			scopedProperties.put("classpath", cp);

			this._delegate.executeMacroInstance(macroDef, "executeJdtProject",
					scopedProperties);
		}
	}
